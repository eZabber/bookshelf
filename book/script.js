/* =========================================================
   ...
   FIXED / CLEANED JS (no Sheets, no Calendar unless you add it back)
   - Removes Calendar discovery + toggle + localStorage key (since HTML removed it)
   - Removes injected CSS <style> block (use your style.css instead)
   - Fixes cloud-controls injection location (put it inside the "Data & Backup" section)
   - Fixes border variable bug (--border-color -> --border)
   - Makes button state + sync-dot consistent
   ========================================================= */
...
// Optional: If you use Drive Picker etc. you might need an API key.
// For drive v3 discovery + upload via fetch, it can be blank.
const DEVELOPER_KEY = "";

// Drive-only scope (hidden app data)
const DRIVE_SCOPE = "https://www.googleapis.com/auth/drive.appdata";

// Only Drive discovery now
...
function setSyncStatus(state) {
  ...
  if (btn) {
    ...
    btn.textContent =
      state === "working" ? t("working") : state === "synced" ? t("synced") : t("signIn");
  }
}
...
function setLanguage(lang) {
  ...
  setText("modal-add-read", `Add → ${t("read")}`);
  setText("modal-add-wish", `Add → ${t("wishlist")}`);
  setText("modal-add-loan", `Add → ${t("loans")}`);
  setText("modal-cancel", t("cancel"));
  setText("label-audio", t("modalAudio"));
  setText("label-return", t("modalReturn"));

  // Cloud buttons (if injected)
...
function renderBooks() {
  ...
  if (term || filterState.year || filterState.month || filterState.rating) {
    visibleItems = allItems.filter((b) => {
      ...
      const matchMonth =
        !filterState.month || (dateStr.length >= 7 && dateStr.substring(5, 7) === filterState.month);
      const matchRating =
  ...
  if (statusEl) {
    if (allItems.length !== visibleItems.length) {
      ...
      statusEl.innerHTML = `
        <span>${msg}</span>
        <span class="filter-clear-link" onclick="clearFilters()">${t("clearBtn")}</span>
      `;
    } else {
  ...
  visibleItems
    ...
    .forEach((b) => {
      ...
      copyBtn.onclick = () =>
        navigator.clipboard?.writeText?.(String(b?.title || ""))?.catch?.(() => {});
      ...
      if (currentShelf === "read") {
        ...
        sel.innerHTML =
          ...
          [1, 2, 3, 4, 5]
            .map(
              (n) =>
                `<option value="${n}" ${currentRating === n ? "selected" : ""}>${"⭐".repeat(
                  n
                )}</option>`
            )
            .join("");
...
function confirmAdd(targetShelf) {
  ...
  if (targetShelf === "loans") {
    ...
    if (!row || !input) {
      toast("Missing loan fields.");
      return;
    }

    // first click reveals date input + sets default
...
function gisLoaded() {
  driveTokenClient = google.accounts.oauth2.initTokenClient({
    ...
    callback: async (resp) => {
      ...
      await findCloudFileIfExists(); // warm up cached file id
    }
  });
...
async function findCloudFileIfExists() {
  ...
  if (cloudFileId) {
    try {
      await gapi.client.drive.files.get({ fileId: cloudFileId, fields: "id" });
      return cloudFileId;
    } catch {
      cloudFileId = null;
...
async function fetchAndPrompt(rawIsbn) {
  ...
  const book =
    (await fetchOpenLibrary(clean)) || (await fetchFinna(clean)) || (await fetchGoogleBooks(clean));
...
async function handleManualAdd() {
  ...
  const looksIsbn = /^[\d\s-]+$/.test(val);
  if (looksIsbn) await fetchAndPrompt(val);
...
function injectCloudControlsIfMissing() {
  // Put cloud controls INSIDE the Data & Backup section
  const dataSectionTitle = $("menu-data");
  const exportBtn = $("btn-export");
  if (!exportBtn) return;

  // Find the Data & Backup section container by walking up from export button
  const dataSection = exportBtn.closest(".menu-section");
  ...
  cloudDiv.innerHTML = `
    <div id="header-drive">${t("headerDrive")}</div>
    ...
    <div id="header-local" style="margin-top: 12px;">${t("headerLocal")}</div>
  `;

  // insert cloud controls above export/import buttons
  dataSection.insertBefore(cloudDiv, exportBtn);
...
window.addEventListener("DOMContentLoaded", () => {
  try {
    ...
    // IMPORTANT: inject after language so labels are translated
    injectCloudControlsIfMissing();

    console.log("App Ready (Drive appData JSON, Drive-only)");
  } catch (e) {
    logError("Init", e);
    setSyncStatus("error");
  }
